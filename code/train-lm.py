...
from src.model.lm import check_lm_params, build_language_model
from src.trainer import TrainerLM
from src.evaluator import EvaluatorLM


# parse parameters
parser = argparse.ArgumentParser(description='Attribute-based language model - Training')
...
parser.add_argument("--lm_hidden_dim", type=int, default=512,
                    help="Number of kernels")
parser.add_argument("--lm_bias", type=bool_flag, default=True,
                    help="Use a bias in the softmax layer")
parser.add_argument("--lm_dropout", type=float, default=0,
                    help="Dropout")
parser.add_argument("--lm_bos_attr", type=str, default='',
                    help="Beginning of sentence attribute embedding")
parser.add_argument("--lm_bias_attr", type=str, default='',
                    help="Attribute bias")
# dataset
...
parser.add_argument("--lm_optimizer", type=str, default="adam,lr=0.0003",
...
if __name__ == '__main__':
    ...
    check_lm_params(params)

    # initialize / load data / build model
    logger = initialize_exp(params)
    data = load_data(params, mono_only=True)
    lm = build_language_model(params, data['dico'])

    # trainer / checkpoint / evaluator
    trainer = TrainerLM(lm, data, params)
    trainer.reload_checkpoint()
    evaluator = EvaluatorLM(trainer, data, params)
    ...
    for _ in range(trainer.epoch, params.max_epoch):
        ...
        while trainer.n_sentences < params.epoch_size:
            trainer.lm_step(attr_label=trainer.get_attr_label())
            trainer.iter()
